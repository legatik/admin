...
routers.App.prototype.syncView = function() {
    // Finally send the page to the client.
    this.res.send(Bones.plugin.templates.App({
        version: time,
        title: this.interface.pageTitle,
        main: '',
        startup: 
        'Bones.initialize();'+
                 'Bones.start({pushState:true, root: "", silent: true});'+
                 'Bones.url = ' + '"' + process.env.NODE_URL + '"; ' + 
                 'Bones.routers.App.state.inflate(' + JSON.stringify(this.state.deflate()) + ');'  
